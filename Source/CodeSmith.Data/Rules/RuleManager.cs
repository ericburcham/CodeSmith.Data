...
namespace CodeSmith.Data.Rules
{
    ...
    public class RuleManager
    {
        ...
        private static readonly object _addSharedRuleLock = new object();

        private static readonly object _addSharedLock = new object();
        ...
        public bool Run(TrackedObject trackedObject)
        {
            ...
            var rules = GetRules(trackedObject.Current.GetType());
            rules.Sort(
        ...
        ///     The <see cref="DataContext" /> to get the <see cref="ChangeSet" /> from and run rules
        ///     against.
        /// </param>
        /// <returns><c>true</c> if rules ran successfully; otherwise, <c>false</c>.</returns>
        ...
        private static void AddValidation(string property, Attribute attribute, Type entityType)
        {
            ...
            else if (attribute is RangeAttribute)
            {
                ...
                if (string.IsNullOrEmpty(validationAttribute.ErrorMessage))
                {
                    var rangeRule = Activator.CreateInstance(
                        typeof(RangeRule<>).MakeGenericType(validationAttribute.OperandType),
                        property,
                        validationAttribute.Minimum,
                        validationAttribute.Maximum) as PropertyRuleBase;
