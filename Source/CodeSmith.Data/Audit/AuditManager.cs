...
namespace CodeSmith.Data.Audit
{
    ...
    public static class AuditManager
    {
        ...
        private const BindingFlags _defaultStaticBinding =
        ...
        private static readonly Dictionary<Type, MetaDataMember> _displayColumnCache =
            new Dictionary<Type, MetaDataMember>();

        private static readonly ReaderWriterLockSlim _formatterLock = new ReaderWriterLockSlim();

        private static readonly Dictionary<MemberInfo, MethodInfo> _formatterCache =
            new Dictionary<MemberInfo, MethodInfo>();
        ...
        public static AuditLog MergeAuditLogs(params AuditLog[] logs)
        {
            ...
            for (var i = 1; i < logs.Length; i++)
            {
                ...
                foreach (var entity in current.Entities)
                {
                    ...
                    var index = mergedLog.Entities.IndexOf(entity);
                    if (index >= 0)
                    {
        ...
        public static void Refresh(AuditLog log)
        {
            ...
            foreach (var auditEntity in log.Entities)
            {
                ...
                var current = auditEntity.Current;
                if (current == null)
                {
                    continue;
                }
                ...
                foreach (var property in auditEntity.Properties.Where(p => p.MetaDataMember != null))
                {
                    try
                    {
                        ...
                        var underlyingType = GetUnderlyingType(dataMember.Type);
        ...
        private static void AddAssociationProperties(
            MetaType metaType,
            ITable table,
            object entity,
            AuditEntity auditEntity)
        {
            foreach (var association in metaType.Associations)
            {
                ...
                try
                {
                    ...
                    var currentChildEntity = thisMember.DeferredValueAccessor.GetBoxedValue(entity);
                    var currentValue =
                        GetAssociationValue(association, displayMember, currentChildEntity, table, entity);
                    ...
                    if (auditEntity.Action == AuditAction.Update && table != null)
                    {
                        ...
                        var original = table.GetOriginalEntityState(entity);
                        if (original != null)
                        {
                            //this will get the fkey entity with out causing a load
                            var originalChildEntity = thisMember.DeferredValueAccessor.GetBoxedValue(original);
                            auditProperty.Original =
                                GetAssociationValue(association, displayMember, originalChildEntity, table, original) ??
                                _nullText;
                        }
                    }
                }
            ...
            IEnumerable<object> entities)
        {
            foreach (var entity in entities)
            {
                ...
                var auditEntity = CreateAuditEntity(dataContext, entity, action);
        ...
        private static void AddAuditKeys(MetaType metaType, object entity, AuditEntity auditEntity)
        {
            foreach (var dataMember in metaType.IdentityMembers)
            {
                ...
                try
                {
                    ...
                    var value = GetKeyValue(dataMember, entity);
        ...
        private static void AddAuditProperties(MetaType metaType, ITable table, object entity, AuditEntity auditEntity)
        {
            ...
            foreach (var dataMember in metaType.PersistentDataMembers)
            {
                ...
                if (auditEntity.Action == AuditAction.Update && modifiedMemberInfo.Member == null &&
                    !HasAlwaysAuditAttribute(dataMember.Member))
                {
        ...
        private static AuditProperty CreateAuditProperty(
            MetaDataMember dataMember,
            ModifiedMemberInfo modifiedMemberInfo,
            object entity,
            AuditEntity auditEntity)
        {
            var auditProperty = new AuditProperty();
            try
            {
                ...
                if (auditEntity.Action == AuditAction.Update && modifiedMemberInfo.Member != null)
                {
                    auditProperty.Current = GetValue(
                        modifiedMemberInfo.Member,
                        underlyingType,
                        modifiedMemberInfo.CurrentValue,
                        entity);

                    auditProperty.Original = GetValue(
                        modifiedMemberInfo.Member,
                        underlyingType,
                        modifiedMemberInfo.OriginalValue,
                        entity);
                ...
                var value = GetValue(
                    dataMember.Member,
                    underlyingType,
                    dataMember.MemberAccessor.GetBoxedValue(entity),
                    entity);
        ...
        private static object GetAssociationValue(
            MetaAssociation association,
            MetaDataMember childDisplayMember,
            object childEntity,
            ITable table,
            object entity)
        {
            ...
            try
            {
                ...
                // build dymamic query
                for (var i = 0; i < association.ThisKey.Count; i++)
                {
                    var v = association.ThisKey[i].MemberAccessor.GetBoxedValue(entity);
            ...
            where TAttribute : Attribute
        {
            ...
            var attribute = memberInfo.GetCustomAttributes(attributeType, true).FirstOrDefault() as TAttribute;
            if (attribute != null)
            {
                return attribute;
            }
            ...
            var metadataTypeAttribute =
                declaringType.GetCustomAttributes(typeof(MetadataTypeAttribute), true).FirstOrDefault() as
            ...
            var metaInfo = metadataType.GetMember(memberInfo.Name, _defaultBinding).FirstOrDefault();
            if (metaInfo == null)
            {
                return null;
            }
        ...
        private static MetaDataMember GetDisplayMember(MetaType rowType)
        {
            ...
            using (_displayColumnLock.ReadLock())
            {
                ...
                using (_displayColumnLock.WriteLock())
                {
                    ...
                    {
                        displayMember =
                            rowType.DataMembers.FirstOrDefault(m => m.Name == displayAttribute.DisplayColumn);
                    }
        ...
        private static object GetValue(MemberInfo memberInfo, Type valueType, object value, object entity)
        {
            ...
            try
            {
                var returnValue = valueType.IsEnum
                    ? Enum.GetName(valueType, value)
                    : value;
                ...
                using (_formatterLock.ReadLock())
                {
                    ...
                    if (!_formatterCache.TryGetValue(memberInfo, out formatMethod))
                    {
                        using (_formatterLock.WriteLock())
                        {
                            ...
                            var formatterType = formatAttribute.FormatType;

                            //first: static object MethodName(MemberInfo memberInfo, object value, object entity)
                            formatMethod = formatterType.GetMethod(
                                formatAttribute.MethodName,
                                _defaultStaticBinding,
                                null,
                                new[] { typeof(MethodInfo), typeof(object), typeof(object) },
                                null);
        ...
        private static bool HasAlwaysAuditAttribute(MemberInfo memberInfo)
        {
            using (_alwaysAuditLock.ReadLock())
            {
                ...
                {
                    var result = HasAttribute(memberInfo, typeof(AlwaysAuditAttribute));
        ...
        private static bool HasAuditAttribute(object entity)
        {
            ...
            using (_auditableLock.ReadLock())
            {
                ...
                {
                    var result = HasAttribute(entityType, typeof(AuditAttribute));
        ...
        private static bool HasNotAuditedAttribute(MemberInfo memberInfo)
        {
            using (_notAuditedLock.ReadLock())
            {
                ...
                {
                    var result = HasAttribute(memberInfo, typeof(NotAuditedAttribute));
