...
namespace CodeSmith.Data.Rules
{
    ...
    public abstract class PropertyRuleBase : IRule
    {
        ...
        public static object CoerceValue(Type desiredType, Type valueType, object value)
        {
            ...
            var isNullable = desiredType.IsGenericType && (desiredType.GetGenericTypeDefinition() == NullableType);
            if (isNullable)
            {
                if (value == null)
                {
            ...
            var isBinary = (desiredType.IsArray && desiredType.Equals(ByteArrayType)) || desiredType.Equals(BinaryType);
            ...
            {
                var bytes = (value is Binary)
                    ? ((Binary)value).ToArray()
                    : (byte[])value;
        ...
        protected object GetPropertyValue(object target)
        {
            ...
            var propertyInfo = GetPropertyInfo(target);
        ...
        {
            var value = GetPropertyValue(target);

            return (value == null)
                ? default(T)
                : (T)CoerceValue(typeof(T), value);
        }
        ...
        protected bool IsPropertyValueModified(object original, object current)
        {
            ...
            var currentValue = GetPropertyValue(current);
            ...
            var originalValue = GetPropertyValue(original);
        ...
        protected void SetPropertyValue(object target, object value)
        {
            ...
            {
                var pType = GetUnderlyingType(propertyInfo.PropertyType);
                var vType = GetUnderlyingType(value.GetType());
